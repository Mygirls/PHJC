...
@interface InvestingViewController ()<UITableViewDelegate, UITableViewDataSource>
...
__weak InvestingViewController *_invSelf;
...
- (void)setRefresh {
    _tableView.mj_header = [WMRefreshHeader headerWithRefreshingBlock:^{
        [_invSelf load_refresh];
    }];
    _tableView.mj_footer = [WMRefreshFooter footerWithRefreshingBlock:^{
        if (_invSelf.is_have_more) {
            [_invSelf load_dingqi_product];
        }else {
            [_invSelf.tableView.mj_footer endRefreshing];
...
{
    //200成功
    [CMCore invest_record_with_status:200 market_type:20 page:_page count:LOAD_COUNT_MAX is_alert:YES blockResult:^(NSNumber *code, id result, NSString *message) {
