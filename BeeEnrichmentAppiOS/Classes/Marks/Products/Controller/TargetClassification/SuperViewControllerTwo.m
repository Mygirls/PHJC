...
@interface SuperViewControllerTwo ()<UITableViewDelegate,UITableViewDataSource,PaymentViewDelegate>
...
@implementation SuperViewControllerTwo
-(void)viewDidLoad{
    [super viewDidLoad];
...
//    _section_view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth,20)];
...
#pragma mark 设置下拉刷新
- (void)set_refresh {
    __weak SuperViewControllerTwo *_self = self;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    if (section == 1) {
        if(_isClose){
            [_section_btn setTitle:@"以下为近期已满额项目" forState:UIControlStateNormal];
            ...
            [_section_btn setTitle:@"查看更多满额项目" forState:UIControlStateNormal];
            [_section_btn.titleLabel sizeToFit];
            titleLabelWidth = _section_btn.titleLabel.bounds.size.width;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_subject_list.count !=0&&_hlist_list.count!=0) {
...
- (void)load_product_list{
    ...
    [CMCore get_subject_list_with_subject_status:20 page:self.pageInteger count:30 is_alert:YES blockResult:^(NSNumber *code, id result, NSString *message) {
        [self.tableView.mj_header endRefreshing];
        [self.tableView.mj_footer endRefreshing];
        if (result) {
            NSMutableArray<ProductsDetailModel *>  *dic = [ProductsDetailModel mj_objectArrayWithKeyValuesArray:result[@"history"][@"hlist"]];
            
            [_hlist_list removeAllObjects];
            [_subject_list removeAllObjects];
            
            [_subject_list addObjectsFromArray:[ProductsDetailModel mj_objectArrayWithKeyValuesArray:result[@"ing"]]];
