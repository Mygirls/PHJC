...
//  BeeEnrichmentAppiOS
//
//  Created by LiuXiaoMin on 16/5/9.
...
- (void)showInView:(UIView*)view iamge_ulr:(NSString *)image_url title:(NSString *)title;
{
    ...
    [UIView animateWithDuration:0.25f animations:^{
        ...
        if (_image_url.length > 0) {
            [_showImageView sd_setImageWithURL:[NSURL URLWithString:_image_url] placeholderImage:[UIImage imageNamed:@"v1_placeholderImage"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                if (!error) {
