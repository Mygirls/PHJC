...
#define kCircleDiameter ([UIScreen mainScreen].bounds.size.width - 176.9) / 3          // 圆点直径 5
#define kCircleMargin  ([UIScreen mainScreen].bounds.size.width - kCircleDiameter*3 - 43.2 - 43.7) / 2              // 圆点离屏幕左边距

#define kLLCircleAlpha 1.0              // 圆点透明度
#define kLLLineWidth 0.5               // 线条宽
//#define kLLLineColor [UIColor colorWithRed:1 green:1 blue:1 alpha:0.36] // 线条色白
#define kLLLineColor [UIColor colorWithHex:@"#ff7357"]
#define kLLLineColorWrong [UIColor colorWithRed:201.0/255.0 green:9.0/255.0 blue:22.0/255.0 alpha:0.3] // 线条色红
...
- (void)initCircles
{
    ...
    for (int i = 0; i < 9; i++) {
        ...
        int x = kCircleMargin + (i%3) * (kCircleDiameter+([UIScreen mainScreen].bounds.size.width-kCircleMargin*2- kCircleDiameter *3)/2);
        int y = kCircleMargin + (i/3) * (kCircleDiameter+([UIScreen mainScreen].bounds.size.width-kCircleMargin*2- kCircleDiameter *3)/2);
//        LLLog(@"每个圆点位置 %d,%d", x, y);
        [button setFrame:CGRectMake(x, y, kCircleDiameter, kCircleDiameter)];
        
        [button setBackgroundColor:[UIColor clearColor]];
        [button setBackgroundImage:[UIImage imageNamed:@"v1.6_circle_normal"] forState:UIControlStateNormal];
        [button setBackgroundImage:[UIImage imageNamed:@"v1.6_circle_selected"] forState:UIControlStateSelected];
...
//    LLLog(@"point x=%.2f, y=%.2f", point.x, point.y);
