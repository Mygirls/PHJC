...
//                {
                    ...
                    if (_delegate && [_delegate respondsToSelector:@selector(passwordView:inputPassword:)]) {
    ...
    if (self)
    {
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:UITextFieldTextDidChangeNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *note){
            ...
            if ([note.object isEqual:_passwordTextField])
            {
                ...
                if (_numbers_count != 0 && _passwordTextField.text.length == _numbers_count)
                {
                    if (_delegate && [_delegate respondsToSelector:@selector(passwordView:inputPassword:)])
                    {
                        [_passwordTextField resignFirstResponder];
...
- (void)initView
{
    ...
    CGFloat width = self.bounds.size.width / _numbers_count;
    
    for (int i = 0; i < _numbers_count - 1; i++)
    {
        UIImageView *lineImageView = [[UIImageView alloc] initWithFrame:CGRectMake((i + 1) * width, kLineMarginTop, 0.5f, self.bounds.size.height - 2 * kLineMarginTop)];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    else if(textField.text.length >= _numbers_count)
    {
