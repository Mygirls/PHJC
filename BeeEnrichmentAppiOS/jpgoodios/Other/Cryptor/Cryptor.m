...
+ (NSString *)parseByteArray2HexString:(Byte[]) bytes withLength:(int)len;
...
+ (NSString *)encodeDES_Hex:(NSString *)plainString key:(NSString *)key {
    NSString *ciphertext = nil;
    NSData *textData = [plainString dataUsingEncoding:NSUTF8StringEncoding];
    ...
    CCCryptorStatus cryptStatus = CCCrypt(kCCEncrypt,
                                          ...
                                          &numBytesEncrypted);
    if (cryptStatus == kCCSuccess) {
        ciphertext = [Cryptor parseByteArray2HexString:buffer withLength:(int)numBytesEncrypted];
    }
    return ciphertext;
}
